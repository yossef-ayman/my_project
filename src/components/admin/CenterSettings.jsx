...
const CenterSettings = ({ availableLocations, availableDays, onUpdateLocations, onUpdateDays, onBack }) => {
  ...
  const addLocation = () => {
    if (!newLocation.trim()) {
      toast({
        title: "خطأ",
        description: "يرجى إدخال اسم المكان",
        variant: "destructive",
      })
      return
    }

    if (availableLocations.includes(newLocation.trim())) {
      toast({
        title: "خطأ",
    ...
    onUpdateLocations([...availableLocations, newLocation.trim()])
    setNewLocation("")
    toast({
      title: "تم الإضافة",
      description: `تم إضافة المكان: ${newLocation.trim()}`,
    })
  }
  ...
  const removeLocation = (location) => {
    ...
    toast({
      title: "تم الحذف",
      description: `تم حذف المكان: ${location}`,
    })
  }
  ...
  const addDay = () => {
    if (!newDay.trim()) {
      toast({
        title: "خطأ",
        description: "يرجى إدخال اسم اليوم",
        variant: "destructive",
      })
      return
    }

    if (availableDays.includes(newDay.trim())) {
      toast({
        title: "خطأ",
    ...
    onUpdateDays([...availableDays, newDay.trim()])
    setNewDay("")
    toast({
      title: "تم الإضافة",
      description: `تم إضافة اليوم: ${newDay.trim()}`,
    })
  }
  ...
  return (
    <div className="space-y-6" dir="rtl">
      ...
      {/* أيام الأسبوع السريعة */}
      <Card>
        <CardHeader>
          <CardTitle>إضافة سريعة لأيام الأسبوع</CardTitle>
          <CardDescription>انقر على الأيام لإضافتها بسرعة</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="flex flex-wrap gap-2">
